...
export function FlashcardMode({ setIdx, customCards, initialIndex, onProgress, onBack, onGoConv, onGoQuiz }) {
  ...
  return (
    <div style={BG}>
      ...
      <p style={{ color:"rgba(255,255,255,0.38)", fontSize:14, marginBottom:10, zIndex:1 }}>Tap card to reveal</p>
      ...
      <div style={{ display:"flex", gap:12, alignItems:"center", zIndex:1, width:"100%", maxWidth:MAX, justifyContent:"center" }}>
        ...
        <button onClick={()=>setFlipped(f=>!f)} style={{ flex:1, maxWidth:180, padding:"12px 0", borderRadius:99, background:`linear-gradient(135deg,${GOLD},${ORANGE})`, border:"none", color:"#1a1500", fontSize:15, fontWeight:700, cursor:"pointer" }}>{flipped?"HIDE":"REVEAL"}</button>
